...
export function renderTimetable() {
  ...
  scheduleData.timePeriods.forEach((period) => {
    ...
    timetableGrid.appendChild(timeCell);    // 为每一天添加课程单元格
    for (let dayId = 1; dayId <= 7; dayId++) {
      ...
      // [调试输出] 单元格创建信息 - TC_SCH_GRID_003 已完成，暂时注释
      // console.log(`[TC_SCH_GRID_003 调试] 创建单元格: 星期${dayId}, 时间段${period.id} (${period.time})`);
      
      // 添加鼠标悬停调试事件 - TC_SCH_GRID_003 已完成，暂时注释
      // cell.addEventListener('mouseenter', function() {
      //   console.log(`[TC_SCH_GRID_003 调试] 鼠标进入单元格: 星期${dayId}, 时间段${period.id}`);
      // });
      
      // cell.addEventListener('mouseleave', function() {
      //   console.log(`[TC_SCH_GRID_003 调试] 鼠标离开单元格: 星期${dayId}, 时间段${period.id}`);
      // });
